...
func TestUnlock(t *testing.T) {
	...
	expected := &corev1.Secret{
		...
		Data: map[string][]byte{
			"test":  []byte{0x74, 0x65, 0x73, 0x74},
			"test1": []byte{0x74, 0x65, 0x73, 0x74, 0x31},
		},
	}

	if err := lb.UnlockInto(secret, priKey); err != nil {
		t.Fatalf("unexpected error unlocking secrets: %v", err)
	}

	if diff := cmp.Diff(secret, expected); diff != "" {
